"""Collect Trend Pullback Continuation forward observations.

This records qualifying Trend Pullback signals from the latest saved candles
into a strategy-specific forward observation journal. It is separate from
official paper trades and from the shadow-sample journal.
...
from run_trend_pullback_continuation_shadow_samples import (
    DEFAULT_SYMBOLS,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect Trend Pullback forward observations.")
    ...
    parser.add_argument(
        "--observations-csv",
        type=Path,
        default=Path("data/trend_pullback_continuation_forward_observations.csv"),
        help="Append-only Trend Pullback forward observation journal.",
    )
...
def shadow_to_observations(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    result["observation_reason"] = "Recent saved candle passed tightened Trend Pullback forward-observation filters."
...
def shadow_outcomes_to_observations(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    result["evaluation_note"] = result["evaluation_note"].astype(str).str.replace(
        "Trend Pullback shadow outcome only",
        "Trend Pullback forward observation outcome only",
        regex=False,
    )
...
def write_report(
    ...
    path.write_text(
        f"""# Trend Pullback Continuation Forward Observations

This report collects forward observations for the Strategy Vault's Trend
Pullback Continuation candidate when the latest saved candles pass the
tightened strategy filters.
...
```text
Trend Pullback forward observations stay separate from official paper trades.
...
def main() -> None:
    ...
    outcomes_csv = args.output_dir / "trend_pullback_continuation_forward_observation_results.csv"
    report_path = args.output_dir / "trend_pullback_continuation_forward_observations.md"
    ...
    print(f"Trend Pullback forward observation status: {append_status}")
