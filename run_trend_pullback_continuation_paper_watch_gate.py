"""Paper-watch gate for Trend Pullback Continuation.

This report answers whether the Strategy Vault's Trend Pullback Continuation
candidate has enough evidence to be considered for manual paper-watch review.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Trend Pullback paper-watch gate.")
...
def count_decisions(frame: pd.DataFrame, decisions: set[str]) -> int:
    """Count rows whose decision is in the allowed set."""

    if frame.empty or "decision" not in frame.columns:
        return 0
    return int(frame["decision"].astype(str).isin(decisions).sum())
...
def build_gate(args: argparse.Namespace) -> tuple[dict[str, Any], pd.DataFrame]:
    ...
    tightened_review = read_csv_or_empty(output_dir / "trend_pullback_continuation_tightened_review.csv")
    walk_forward = read_csv_or_empty(output_dir / "trend_pullback_continuation_walk_forward.csv")
    shadow = read_csv_or_empty(output_dir / "trend_pullback_continuation_shadow_outcomes.csv")
    forward = read_csv_or_empty(output_dir / "trend_pullback_continuation_forward_observation_results.csv")

    tightened_count = count_decisions(
        tightened_review,
        {"passes_tightened_research", "provisional_tightened_pass"},
    )
    final_tightened_count = count_decisions(tightened_review, {"passes_tightened_research"})
    provisional_tightened_count = count_decisions(tightened_review, {"provisional_tightened_pass"})
    walk_holding = (
    ...
    rows = [
        gate_row(
            ...
            tightened_count >= args.min_tightened_pass_rows,
            "At least one final or provisional tightened-review row must pass before paper-watch review.",
        ),
        gate_row(
    ...
    payload = {
        ...
        "tightened_pass_rows": int(tightened_count),
        "final_tightened_pass_rows": int(final_tightened_count),
        "provisional_tightened_pass_rows": int(provisional_tightened_count),
...
def write_report(path: Path, payload: dict[str, Any], checklist: pd.DataFrame) -> None:
    ...
    path.write_text(
        f"""# Trend Pullback Continuation Paper-Watch Gate

This report decides whether Trend Pullback Continuation has enough evidence to
be considered for manual paper-watch review.
...
```text
Tightened pass rows: {payload["tightened_pass_rows"]} ({payload["final_tightened_pass_rows"]} final / {payload["provisional_tightened_pass_rows"]} provisional)
...
def main() -> None:
    ...
    json_path = args.output_dir / "trend_pullback_continuation_paper_watch_gate.json"
    csv_path = args.output_dir / "trend_pullback_continuation_paper_watch_gate.csv"
    md_path = args.output_dir / "trend_pullback_continuation_paper_watch_gate.md"
    json_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    checklist.to_csv(csv_path, index=False)
    write_report(md_path, payload, checklist)
    print(f"Trend Pullback paper-watch decision: {payload['decision']}")
