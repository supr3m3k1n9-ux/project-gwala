"""Walk-forward review for VWAP mean-reversion research.

This report checks whether the newest mean-reversion trades still support the
older backtest sample. It is research-only: it does not create scanner
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build VWAP mean-reversion walk-forward review.")
    parser.add_argument("--output-dir", type=Path, default=Path("logs"), help="Where reports are saved.")
    parser.add_argument(
        "--trade-log",
        type=Path,
        default=Path("logs/vwap_mean_reversion_trades.csv"),
        help="Consolidated mean-reversion trade log.",
    )
    parser.add_argument(
        "--summary-csv",
        type=Path,
        default=Path("logs/vwap_mean_reversion_summary.csv"),
        help="Mean-reversion summary rows to review.",
    )
...
def review_row(summary_row: pd.Series, trades: pd.DataFrame, args: argparse.Namespace) -> dict[str, Any]:
    """Build one walk-forward review row from a mean-reversion summary row."""
...
def build_review(summary: pd.DataFrame, trades: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    """Build walk-forward rows for useful mean-reversion summary rows."""
...
def write_outputs(output_dir: Path, review: pd.DataFrame, args: argparse.Namespace) -> None:
    ...
    csv_path = output_dir / "vwap_mean_reversion_walk_forward.csv"
    json_path = output_dir / "vwap_mean_reversion_walk_forward.json"
    md_path = output_dir / "vwap_mean_reversion_walk_forward.md"
    ...
    if review.empty:
        body = "No promising mean-reversion rows were available for walk-forward review."
    else:
...
"""
    ...
    md_path.write_text(
        f"""# VWAP Mean Reversion Walk-Forward Review

This report splits each promising VWAP mean-reversion row into an older half and
a newer half. The goal is to see whether the newer trades still support the
strategy before it gets closer to forward paper validation.
...
```text
logs/vwap_mean_reversion_walk_forward.json
logs/vwap_mean_reversion_walk_forward.csv
...
def main() -> None:
    ...
    print(f"Saved mean reversion walk-forward CSV: {args.output_dir / 'vwap_mean_reversion_walk_forward.csv'}")
    print(f"Saved mean reversion walk-forward report: {args.output_dir / 'vwap_mean_reversion_walk_forward.md'}")
