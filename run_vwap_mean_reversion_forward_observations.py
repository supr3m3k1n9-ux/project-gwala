"""Collect VWAP mean-reversion forward observations.

This records qualifying VWAP mean-reversion signals from the latest saved
candles into a strategy-specific forward observation journal. It is separate
from official paper trades and from the generic scanner observation journal.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect VWAP mean-reversion forward observations.")
    ...
    parser.add_argument(
        "--observations-csv",
        type=Path,
        default=Path("data/vwap_mean_reversion_forward_observations.csv"),
        help="Append-only mean-reversion forward observation journal.",
    )
...
def shadow_outcomes_to_observations(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    result["evaluation_note"] = result["evaluation_note"].astype(str).str.replace(
        "Mean-reversion shadow outcome only",
        "Mean-reversion forward observation outcome only",
        regex=False,
    )
...
def write_report(
    ...
    path.write_text(
        f"""# VWAP Mean Reversion Forward Observations

This report collects forward observations for the Strategy Vault's VWAP Mean
Reversion candidate when the latest saved candles pass the tightened strategy
filters.
...
```text
Mean-reversion forward observations stay separate from official paper trades.
...
def main() -> None:
    ...
    outcomes_csv = args.output_dir / "vwap_mean_reversion_forward_observation_results.csv"
    report_path = args.output_dir / "vwap_mean_reversion_forward_observations.md"
