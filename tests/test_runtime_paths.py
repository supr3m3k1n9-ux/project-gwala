...
class RuntimePathTests(unittest.TestCase):
    ...
    def test_refresh_audit_related_defaults_use_runtime_data_in_docker(self) -> None:
        ...
        with patch.dict(os.environ, {}, clear=True), patch.object(runtime_paths, "running_in_docker", return_value=True):
            ...
            from run_opening_range_breakout_forward_observations import parse_args as parse_orb_forward_args
            ...
            with patch("sys.argv", ["run_opening_range_breakout_forward_observations.py"]):
                self.assertEqual(
                    parse_orb_forward_args().observations_csv,
                    Path("/app/runtime_data/opening_range_breakout_forward_observations.csv"),
                )
    ...
    def test_dashboard_systemd_service_publishes_compose_service_ports(self) -> None:
        ...
        self.assertIn("docker compose -f /srv/projects/gwala/compose.yaml up --no-build gwala", text)
        self.assertNotIn("run --rm --service-ports", text)
