"""Collect Trend Pullback Continuation shadow samples.

This is the forward-style evidence lane for the Strategy Vault's Trend
Pullback Continuation candidate. It records qualifying pullback-continuation
signals from saved Webull candles, then grades them after complete 5m session
data exists.

Important: these rows are not paper trades, broker alerts, or execution
instructions. They do not count toward official paper-trade gates.
"""
...
from run_trend_pullback_continuation import add_research_columns, find_pullback_exit
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect Trend Pullback Continuation shadow samples.")
    ...
    parser.add_argument(
        "--shadow-csv",
        type=Path,
        default=Path("data/trend_pullback_continuation_shadow_samples.csv"),
        help="Append-only Trend Pullback shadow journal.",
    )
    ...
    parser.add_argument("--target-r-multiple", type=float, default=1.50)
    parser.add_argument("--reward-multiple-floor", type=float, default=0.90)
...
def passes_tightened_filters(row: pd.Series, args: argparse.Namespace) -> bool:
    ...
    quality_score = int(number_value(row.get("trend_pullback_quality_score")))
    relative_volume = number_value(row.get("trend_pullback_relative_volume"))
    trend_gap_pct = number_value(row.get("trend_pullback_trend_gap_pct"))
    vwap_gap_pct = number_value(row.get("trend_pullback_vwap_gap_pct"))
    return (
...
def sample_row(
    *,
    ...
    if plan is None:
        return None
    entry_time = timestamp.tz_convert(MARKET_TZ) if getattr(timestamp, "tzinfo", None) else timestamp
    return {
        "observed_at_et": observed_at_et,
        "scan_date": entry_time.strftime("%Y-%m-%d") if hasattr(entry_time, "strftime") else str(timestamp.date()),
        "entry_time_et": entry_time.strftime("%Y-%m-%d %H:%M") if hasattr(entry_time, "strftime") else str(timestamp),
        "symbol": symbol,
        "strategy": "trend_pullback_continuation",
        "direction": direction,
        "signal_column": signal_column,
        "shadow_status": "strategy_shadow_candidate",
        "shadow_reason": "Recent saved candle passed tightened Trend Pullback shadow filters.",
        **plan,
        "quality_score": int(number_value(row.get("trend_pullback_quality_score"))),
        "quality_grade": str(row.get("trend_pullback_quality_grade", "")),
        "relative_volume": round(number_value(row.get("trend_pullback_relative_volume")), 4),
        "vwap_gap_pct": round(number_value(row.get("trend_pullback_vwap_gap_pct")), 4),
        "trend_gap_pct": round(number_value(row.get("trend_pullback_trend_gap_pct")), 4),
...
def latest_signal_samples_for_symbol(symbol: str, args: argparse.Namespace, observed_at_et: str) -> pd.DataFrame:
    """Return recent Trend Pullback shadow samples for one symbol."""
    ...
    for timestamp, row in recent.iterrows():
        for direction, signal_column in [
            ("long", "trend_pullback_long_signal"),
            ("short", "trend_pullback_short_signal"),
        ]:
            sample = sample_row(
                symbol=symbol,
                timestamp=timestamp,
...
def grade_sample(row: pd.Series, args: argparse.Namespace, cache: dict[str, tuple[pd.DataFrame, pd.DataFrame]]) -> dict[str, Any]:
    """Grade one Trend Pullback shadow sample."""
    ...
    result.update(
        {
            ...
            "evaluation_note": "Trend Pullback shadow outcome only; not an official paper trade.",
        }
    )
    return result
...
def build_outcomes(samples: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    """Grade all stored Trend Pullback shadow samples."""
...
def write_report(
    ...
    """Write a readable Trend Pullback shadow report."""
    ...
    path.write_text(
        f"""# Trend Pullback Continuation Shadow Samples
...
```text
Trend Pullback shadow samples stay separate from official paper trades.
...
def main() -> None:
    ...
    outcomes_csv = args.output_dir / "trend_pullback_continuation_shadow_outcomes.csv"
    report_path = args.output_dir / "trend_pullback_continuation_shadow_samples.md"
    ...
    print(f"Trend Pullback shadow append status: {append_status}")
