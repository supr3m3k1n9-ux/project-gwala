"""Backtest VWAP Reclaim / Reject as a Strategy Vault candidate.

This is research-only. It reuses saved Webull candles and writes reports so we
can decide whether VWAP control flips deserve deeper development. It does not
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backtest VWAP Reclaim / Reject research strategy.")
    ...
    parser.add_argument("--target-r-multiple", type=float, default=1.25, help="Initial VWAP reclaim/reject target in R.")
    parser.add_argument("--reward-multiple-floor", type=float, default=0.80, help="Minimum target distance in R.")
    parser.add_argument("--min-quality-score", type=int, default=4, help="Minimum VWAP reclaim/reject quality score.")
...
def add_research_columns(entry: pd.DataFrame, exit_candles: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    return add_vwap_reclaim_reject_signals(entry, STRATEGY), exit_candles
    ...
    *,
    ...
    for timestamp, row in future.iterrows():
        ...
        if direction == "long":
            if row["low"] <= stop:
                exit_price = stop
                exit_reason = "vwap_reclaim_stop_5m"
            elif row["high"] >= target:
                exit_price = target
                exit_reason = "vwap_reclaim_target_5m"
        else:
            if row["high"] >= stop:
                exit_price = stop
                exit_reason = "vwap_reject_stop_5m"
            elif row["low"] <= target:
                exit_price = target
                exit_reason = "vwap_reject_target_5m"
...
def simulate_direction(
    ...
    *,
    ...
    """Simulate one VWAP reclaim/reject direction."""
    ...
    for index, (timestamp, row) in enumerate(entry_candles.iterrows()):
        ...
        quality_score = int(row.get("vwap_rr_quality_score", 0) or 0)
        relative_volume = float(row.get("vwap_rr_relative_volume", 0) or 0)
        vwap_gap_pct = float(row.get("vwap_rr_gap_pct", 0) or 0)
        trend_gap_pct = float(row.get("vwap_rr_trend_gap_pct", 0) or 0)
        if (
        ...
        if direction == "long":
            stop = min(float(row["low"]), float(row["vwap"])) * (1 - STRATEGY.stop_buffer_pct)
            risk_per_share = entry - stop
            target = entry + (risk_per_share * args.target_r_multiple)
        else:
            stop = max(float(row["high"]), float(row["vwap"])) * (1 + STRATEGY.stop_buffer_pct)
        ...
        trades.append(
            {
                ...
                "quality_grade": str(row.get("vwap_rr_quality_grade", "")),
...
def research_status(metrics: dict[str, Any]) -> str:
    """Classify whether a VWAP reclaim/reject row deserves deeper research."""
...
def promotion_review_status(metrics: dict[str, Any], args: argparse.Namespace) -> tuple[str, str]:
    """Return pass/fail review for a VWAP reclaim/reject row."""
...
def run_symbol(symbol: str, output_dir: Path, args: argparse.Namespace) -> tuple[pd.DataFrame, list[dict[str, Any]]]:
    """Run VWAP reclaim/reject research for one symbol."""
    ...
    output_stem = f"{symbol}_vwap_reclaim_reject_{args.entry_timeframe}_entry_{args.exit_timeframe}_exit"
    ...
    long_trades = simulate_direction(entry, exits, direction="long", signal_column="vwap_reclaim_long_signal", args=args)
    short_trades = simulate_direction(entry, exits, direction="short", signal_column="vwap_reject_short_signal", args=args)
...
def write_report(output_dir: Path, summary: pd.DataFrame, all_trades: pd.DataFrame, args: argparse.Namespace) -> None:
    ...
    (output_dir / "vwap_reclaim_reject.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
    summary.to_csv(output_dir / "vwap_reclaim_reject_summary.csv", index=False)
    all_trades.to_csv(output_dir / "vwap_reclaim_reject_trades.csv", index=False)
    (output_dir / "vwap_reclaim_reject.md").write_text(
        f"""# VWAP Reclaim / Reject Research

This report tests the VWAP control-flip strategy in the Strategy Vault.
...
Long trigger: prior candle below VWAP, current candle touches/reclaims VWAP, EMA 9 >= EMA 21
Short trigger: prior candle above VWAP, current candle touches/rejects VWAP, EMA 9 <= EMA 21
Target: fixed {args.target_r_multiple}R continuation target
Stop: signal candle / VWAP plus buffer
...
```text
logs/vwap_reclaim_reject.json
logs/vwap_reclaim_reject_summary.csv
logs/vwap_reclaim_reject_trades.csv
...
def main() -> None:
    ...
    print(f"Saved VWAP reclaim/reject summary: {args.output_dir / 'vwap_reclaim_reject_summary.csv'}")
    print(f"Saved VWAP reclaim/reject report: {args.output_dir / 'vwap_reclaim_reject.md'}")
