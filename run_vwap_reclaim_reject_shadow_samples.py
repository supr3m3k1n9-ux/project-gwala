"""Collect VWAP reclaim/reject shadow samples.

This is the forward-style observation lane for the Strategy Vault's VWAP
Reclaim / Reject candidate. It records qualifying VWAP control-flip signals
from saved Webull candles, then grades them after complete 5m session data
exists.
...
from run_vwap_reclaim_reject import add_research_columns, find_vwap_exit
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect VWAP reclaim/reject shadow samples.")
    ...
    parser.add_argument(
        "--shadow-csv",
        type=Path,
        default=Path("data/vwap_reclaim_reject_shadow_samples.csv"),
        help="Append-only VWAP reclaim/reject shadow journal.",
    )
...
def passes_tightened_filters(row: pd.Series, args: argparse.Namespace) -> bool:
    ...
    quality_score = int(number_value(row.get("vwap_rr_quality_score")))
    relative_volume = number_value(row.get("vwap_rr_relative_volume"))
    vwap_gap_pct = number_value(row.get("vwap_rr_gap_pct"))
    trend_gap_pct = number_value(row.get("vwap_rr_trend_gap_pct"))
    return (
...
def plan_for_row(row: pd.Series, direction: str, args: argparse.Namespace) -> dict[str, Any] | None:
    """Return the VWAP reclaim/reject plan for a signal row."""
...
def sample_row(
    *,
    ...
    return {
        ...
        "shadow_reason": "Recent saved candle passed tightened VWAP reclaim/reject shadow filters.",
        **plan,
        "quality_score": int(number_value(row.get("vwap_rr_quality_score"))),
        "quality_grade": str(row.get("vwap_rr_quality_grade", "")),
        "relative_volume": round(number_value(row.get("vwap_rr_relative_volume")), 4),
        "vwap_gap_pct": round(number_value(row.get("vwap_rr_gap_pct")), 4),
        "trend_gap_pct": round(number_value(row.get("vwap_rr_trend_gap_pct")), 4),
...
def latest_signal_samples_for_symbol(symbol: str, args: argparse.Namespace, observed_at_et: str) -> pd.DataFrame:
    """Return recent VWAP reclaim/reject shadow samples for one symbol."""
    ...
    for timestamp, row in recent.iterrows():
        for direction, signal_column in [
            ("long", "vwap_reclaim_long_signal"),
            ("short", "vwap_reject_short_signal"),
        ]:
            sample = sample_row(
                symbol=symbol,
                timestamp=timestamp,
...
def grade_sample(row: pd.Series, args: argparse.Namespace, cache: dict[str, tuple[pd.DataFrame, pd.DataFrame]]) -> dict[str, Any]:
    """Grade one VWAP reclaim/reject shadow sample."""
    ...
    result.update(
        {
            ...
            "evaluation_note": "VWAP reclaim/reject shadow outcome only; not an official paper trade.",
        }
    )
    return result
...
def build_outcomes(samples: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    """Grade all stored VWAP reclaim/reject shadow samples."""
...
def write_report(
    ...
    """Write a readable VWAP reclaim/reject shadow report."""
...
```text
VWAP reclaim/reject shadow samples stay separate from official paper trades.
...
def main() -> None:
    ...
    outcomes_csv = args.output_dir / "vwap_reclaim_reject_shadow_outcomes.csv"
    report_path = args.output_dir / "vwap_reclaim_reject_shadow_samples.md"
    ...
    print(f"VWAP reclaim/reject shadow append status: {append_status}")
