...
def preferred_candle_path(data_dir: Path, symbol: str, timeframe: str) -> Path:
    """Return canonical candle path with legacy alias fallback.

    This local stdlib implementation keeps the data-trust runner usable on the
    VPS host, where the application container has pandas but host Python does
    not.
    """

    canonical = data_dir / "candles" / symbol.upper() / f"{timeframe.upper()}.csv"
    if canonical.exists():
        return canonical
