"""Backtest Trend Pullback Continuation as a Strategy Vault candidate.

This is research-only. It reuses saved Webull candles and writes reports so we
can decide whether pullback continuation deserves deeper development. It does
not create scanner candidates, import paper trades, place orders, or connect to
broker execution.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backtest Trend Pullback Continuation research strategy.")
    ...
    parser.add_argument("--target-r-multiple", type=float, default=1.50, help="Initial pullback-continuation target in R.")
    parser.add_argument("--reward-multiple-floor", type=float, default=0.90, help="Minimum target distance in R.")
    parser.add_argument("--min-quality-score", type=int, default=4, help="Minimum trend-pullback quality score.")
    parser.add_argument("--min-relative-volume", type=float, default=0.70)
    parser.add_argument("--max-relative-volume", type=float, default=2.40)
    parser.add_argument("--max-trend-gap-pct", type=float, default=0.0100)
    parser.add_argument("--max-vwap-gap-pct", type=float, default=0.0200)
...
def add_research_columns(entry: pd.DataFrame, exit_candles: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Add indicators, session context, higher timeframe bias, and signals."""
    ...
    return add_trend_pullback_continuation_signals(entry, STRATEGY), exit_candles


def find_pullback_exit(
    *,
    direction: str,
    ...
    for timestamp, row in future.iterrows():
        ...
        if direction == "long":
            if row["low"] <= stop:
                exit_price = stop
                exit_reason = "trend_pullback_stop_5m"
            elif row["high"] >= target:
                exit_price = target
                exit_reason = "trend_pullback_target_5m"
        else:
            if row["high"] >= stop:
                exit_price = stop
                exit_reason = "trend_pullback_stop_5m"
...
def simulate_direction(
    ...
    *,
    ...
    """Simulate one trend-pullback direction."""
    ...
    for index, (timestamp, row) in enumerate(entry_candles.iterrows()):
        ...
        quality_score = int(row.get("trend_pullback_quality_score", 0) or 0)
        relative_volume = float(row.get("trend_pullback_relative_volume", 0) or 0)
        trend_gap_pct = float(row.get("trend_pullback_trend_gap_pct", 0) or 0)
        vwap_gap_pct = float(row.get("trend_pullback_vwap_gap_pct", 0) or 0)
        if (
            ...
            or vwap_gap_pct > args.max_vwap_gap_pct
        ):
            continue
        if (
        ...
        if direction == "long":
            stop = min(float(row["low"]), float(row[slow])) * (1 - STRATEGY.stop_buffer_pct)
            risk_per_share = entry - stop
            target = entry + (risk_per_share * args.target_r_multiple)
        else:
            stop = max(float(row["high"]), float(row[slow])) * (1 + STRATEGY.stop_buffer_pct)
        ...
        exit_result = find_pullback_exit(
            direction=direction,
        ...
        trades.append(
            {
                ...
                "ema_fast": round(float(row[fast]), 4),
                "ema_slow": round(float(row[slow]), 4),
...
def research_status(metrics: dict[str, Any]) -> str:
    """Classify whether a trend-pullback row deserves deeper research."""
...
def promotion_review_status(metrics: dict[str, Any], args: argparse.Namespace) -> tuple[str, str]:
    """Return pass/fail review for a trend-pullback row."""
...
def run_symbol(symbol: str, output_dir: Path, args: argparse.Namespace) -> tuple[pd.DataFrame, list[dict[str, Any]]]:
    """Run trend-pullback research for one symbol."""
    ...
    output_stem = f"{symbol}_trend_pullback_continuation_{args.entry_timeframe}_entry_{args.exit_timeframe}_exit"
    ...
    long_trades = simulate_direction(entry, exits, direction="long", signal_column="trend_pullback_long_signal", args=args)
    short_trades = simulate_direction(entry, exits, direction="short", signal_column="trend_pullback_short_signal", args=args)
...
def write_report(output_dir: Path, summary: pd.DataFrame, all_trades: pd.DataFrame, args: argparse.Namespace) -> None:
    ...
    (output_dir / "trend_pullback_continuation.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
    summary.to_csv(output_dir / "trend_pullback_continuation_summary.csv", index=False)
    all_trades.to_csv(output_dir / "trend_pullback_continuation_trades.csv", index=False)
    (output_dir / "trend_pullback_continuation.md").write_text(
        f"""# Trend Pullback Continuation Research

This report tests the second-chance trend-entry strategy in the Strategy Vault.
...
Long trigger: above VWAP, above EMA 200, EMA 9 >= EMA 21, pullback touches EMA 9/21 band, close recovers above EMA 9
Short trigger: below VWAP, below EMA 200, EMA 9 <= EMA 21, pullback touches EMA 9/21 band, close rejects below EMA 9
Target: fixed {args.target_r_multiple}R continuation target
Stop: signal candle / EMA 21 plus buffer
...
Maximum VWAP gap: {args.max_vwap_gap_pct:.4%}
```
...
```text
logs/trend_pullback_continuation.json
logs/trend_pullback_continuation_summary.csv
logs/trend_pullback_continuation_trades.csv
...
def main() -> None:
    ...
    print(f"Saved trend pullback summary: {args.output_dir / 'trend_pullback_continuation_summary.csv'}")
    print(f"Saved trend pullback report: {args.output_dir / 'trend_pullback_continuation.md'}")
