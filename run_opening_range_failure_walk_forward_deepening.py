"""Deepen Opening Range Failure walk-forward review.

The generic walk-forward matrix is intentionally conservative. This report
adds a strategy-specific provisional durability layer for Opening Range Failure
so promising rows with small samples can keep collecting shadow/forward
evidence without being promoted to paper-watch.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Deepen Opening Range Failure walk-forward review.")
    ...
    parser.add_argument("--min-newer-expectancy-r", type=float, default=0.10)
...
def deepening_row(row: pd.Series, tightened_decisions: dict[tuple[str, str], str], args: argparse.Namespace) -> dict[str, Any]:
    """Return one strategy-specific durability review row."""
    ...
    if not final_blockers:
        ...
        next_action = "Collect OR Failure shadow/forward samples, then re-run this review after more trades mature."
    else:
        decision = "needs_more_walk_forward_evidence"
        blockers = provisional_blockers
        next_action = "Keep in research and wait for more historical/live OR Failure observations."
    ...
    return {
        ...
        "direction": direction,
        "decision": decision,
        "generic_walk_forward_decision": str(row.get("decision", "")),
...
def build_review(output_dir: Path, args: argparse.Namespace) -> tuple[dict[str, Any], pd.DataFrame]:
    """Build Opening Range Failure deep walk-forward review."""

    walk_forward = read_csv_or_empty(output_dir / "opening_range_failure_walk_forward.csv")
    tightened = read_csv_or_empty(output_dir / "opening_range_failure_tightened_review.csv")
    ...
    if walk_forward.empty:
        ...
        order = {
            "walk_forward_pass": 0,
            "provisional_walk_forward_pass": 1,
            "needs_more_walk_forward_evidence": 2,
        }
    ...
    payload = {
        ...
        "next_action": (
            "Collect Opening Range Failure shadow/forward evidence for provisional pass rows."
            if not final_pass.empty or not provisional.empty
            else "Keep Opening Range Failure in research until walk-forward evidence improves."
        ),
        "guardrail": (
...
def write_outputs(output_dir: Path, payload: dict[str, Any], rows: pd.DataFrame) -> None:
    ...
    json_path = output_dir / "opening_range_failure_walk_forward_deepening.json"
    csv_path = output_dir / "opening_range_failure_walk_forward_deepening.csv"
    md_path = output_dir / "opening_range_failure_walk_forward_deepening.md"
    json_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    rows.to_csv(csv_path, index=False)
    md_path.write_text(
        f"""# Opening Range Failure Walk-Forward Deepening

This report checks whether Opening Range Failure has enough newer-slice
...
def main() -> None:
    ...
    print(f"Opening Range Failure walk-forward deepening: {payload['status']}")
