"""Paper-watch gate for VWAP reclaim/reject.

This report answers whether the Strategy Vault's VWAP Reclaim / Reject
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build VWAP reclaim/reject paper-watch gate.")
...
def build_gate(args: argparse.Namespace) -> tuple[dict[str, Any], pd.DataFrame]:
    ...
    summary = read_csv_or_empty(output_dir / "vwap_reclaim_reject_summary.csv")
    walk_forward = read_csv_or_empty(output_dir / "vwap_reclaim_reject_walk_forward.csv")
    shadow = read_csv_or_empty(output_dir / "vwap_reclaim_reject_shadow_outcomes.csv")
    forward = read_csv_or_empty(output_dir / "vwap_reclaim_reject_forward_observation_results.csv")
...
def write_report(path: Path, payload: dict[str, Any], checklist: pd.DataFrame) -> None:
    ...
    path.write_text(
        f"""# VWAP Reclaim / Reject Paper-Watch Gate

This report decides whether VWAP Reclaim / Reject has enough evidence to be
...
def main() -> None:
    ...
    json_path = args.output_dir / "vwap_reclaim_reject_paper_watch_gate.json"
    csv_path = args.output_dir / "vwap_reclaim_reject_paper_watch_gate.csv"
    md_path = args.output_dir / "vwap_reclaim_reject_paper_watch_gate.md"
    json_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    checklist.to_csv(csv_path, index=False)
    write_report(md_path, payload, checklist)
    print(f"VWAP reclaim/reject paper-watch decision: {payload['decision']}")
