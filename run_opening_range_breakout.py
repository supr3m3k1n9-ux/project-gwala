"""Backtest Opening Range Breakout as a Strategy Vault candidate.

This is research-only. It reuses saved Webull candles and writes reports so we
can decide whether OR breakout deserves deeper development. It does not create
...
@dataclass
class OpeningRangeBreakoutTrade:
    """One simulated opening-range breakout trade."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backtest Opening Range Breakout research strategy.")
    ...
    parser.add_argument("--target-r-multiple", type=float, default=1.20, help="Initial breakout target in R.")
    parser.add_argument("--reward-multiple-floor", type=float, default=0.80, help="Minimum target distance in R.")
    parser.add_argument("--min-quality-score", type=int, default=4, help="Minimum OR breakout quality score.")
...
def add_research_columns(entry: pd.DataFrame, exit_candles: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Add indicators, session context, opening range, and OR breakout signals."""
    ...
    return add_opening_range_breakout_signals(entry, STRATEGY), exit_candles


def find_breakout_exit(
    *,
    direction: str,
    ...
    for timestamp, row in future.iterrows():
        ...
        if direction == "long":
            if row["low"] <= stop:
                exit_price = stop
                exit_reason = "or_breakout_stop_5m"
            elif row["high"] >= target:
                exit_price = target
                exit_reason = "or_breakout_target_5m"
        else:
            if row["high"] >= stop:
                exit_price = stop
                exit_reason = "or_breakout_stop_5m"
...
def simulate_direction(
    ...
    *,
    ...
    """Simulate one OR breakout direction."""

    trades: list[OpeningRangeBreakoutTrade] = []
    active_until = None
    trades_today = 0
    daily_r = 0.0
    ...
    for index, (timestamp, row) in enumerate(entry_candles.iterrows()):
        ...
        quality_score = int(row.get("or_breakout_quality_score", 0) or 0)
        relative_volume = float(row.get("or_breakout_relative_volume", 0) or 0)
        trend_gap_pct = float(row.get("or_breakout_trend_gap_pct", 0) or 0)
        if (
        ...
        exit_result = find_breakout_exit(
            direction=direction,
        ...
        trades.append(
            OpeningRangeBreakoutTrade(
                ...
                quality_grade=str(row.get("or_breakout_quality_grade", "")),
...
def research_status(metrics: dict[str, Any]) -> str:
    """Classify whether an OR breakout row deserves deeper research."""
...
def promotion_review_status(metrics: dict[str, Any], args: argparse.Namespace) -> tuple[str, str]:
    """Return pass/fail review for an OR breakout row."""
...
def run_symbol(symbol: str, output_dir: Path, args: argparse.Namespace) -> tuple[pd.DataFrame, list[dict[str, Any]]]:
    """Run OR breakout research for one symbol."""
    ...
    output_stem = f"{symbol}_opening_range_breakout_{args.entry_timeframe}_entry_{args.exit_timeframe}_exit"
    ...
    long_trades = simulate_direction(entry, exits, direction="long", signal_column="or_breakout_long_signal", args=args)
    short_trades = simulate_direction(entry, exits, direction="short", signal_column="or_breakout_short_signal", args=args)
...
def write_report(output_dir: Path, summary: pd.DataFrame, all_trades: pd.DataFrame, args: argparse.Namespace) -> None:
    ...
    (output_dir / "opening_range_breakout.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
    summary.to_csv(output_dir / "opening_range_breakout_summary.csv", index=False)
    all_trades.to_csv(output_dir / "opening_range_breakout_trades.csv", index=False)
    (output_dir / "opening_range_breakout.md").write_text(
        f"""# Opening Range Breakout Research

This report tests the momentum-breakout strategy in the Strategy Vault.
...
Long trigger: close above OR high, above VWAP, EMA 9 >= EMA 21
Short trigger: close below OR low, below VWAP, EMA 9 <= EMA 21
Target: fixed {args.target_r_multiple}R breakout target
Stop: signal candle / opening-range extreme plus buffer
...
```text
logs/opening_range_breakout.json
logs/opening_range_breakout_summary.csv
logs/opening_range_breakout_trades.csv
...
def main() -> None:
    ...
    print(f"Saved OR breakout summary: {args.output_dir / 'opening_range_breakout_summary.csv'}")
    print(f"Saved OR breakout report: {args.output_dir / 'opening_range_breakout.md'}")
